...
We will see a quick setup to configure the ``tool_brew`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_brew`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_brew`` formulas under ``/srv/salt/tool_brew/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_brew/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_brew/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_brew/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_brew/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_brew`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_brew`` formulas under ``/srv/salt/tool_brew/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_brew/parameters/roles
...
We create a configuration for the role ``tool_brew/server`` in ``/srv/salt/tool_brew/parameters/roles/tool_brew/server.yaml``:
...
We create another configuration for the role ``tool_brew/client`` in ``/srv/salt/tool_brew/parameters/roles/tool_brew/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_brew/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_brew/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_brew/parameters/dns:domain/  '/srv/salt/tool_brew/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-brew-as-json:
      test.nop:
        - name: {{ brew | json }}
...
.. code-block:: sls
    ...
    brew-config-file-file-managed:
      file.managed:
        - name: {{ brew.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='brew-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ brew.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``brew`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ brew['winner'] }}
